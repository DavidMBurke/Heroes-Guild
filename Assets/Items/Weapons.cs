...
public class Weapons
{
    public class Melee
    {
        public static Item CreateDagger(Item metalIngot, Item gripMaterial)
        {
            ...
            Item dagger = new Item(name, cost, multiplier: multiplier, description: description);
        ...
        public static Item CreateShortsword(Item metalIngot, Item gripMaterial)
        {
            string name = $"{metalIngot.itemName} Shortsword".Replace(" Ingot", "");
            string description = $"{name} with {gripMaterial.itemName} Grip".Replace(" Ingot", "");
            float multiplier = metalIngot.multiplier * gripMaterial.multiplier;
            int cost = metalIngot.cost + gripMaterial.cost;
            Item shortsword = new Item(name, cost, multiplier: multiplier, description: description);
            shortsword.craftingIngredients = new List<Item>
                {
                    metalIngot, gripMaterial
                };
            shortsword.skillBonuses = metalIngot.skillBonuses.Concat(gripMaterial.skillBonuses).ToDictionary(kvp => kvp.Key, kvp => kvp.Value);
            shortsword.skillMultipliers = metalIngot.skillMultipliers.Concat(gripMaterial.skillMultipliers).ToDictionary(kvp => kvp.Key, kvp => kvp.Value);
            shortsword.tags = new List<string> { "melee", "weapon", "sword" };
            shortsword.equipSlots.Add(EquipmentSlots.Enum.RightHand);
            shortsword.SetModifiers();
    ...
    public class Ranged
    {
        public static Item CreateShortbow(Item wood, Item bowString)
        {
            ...
            Item bow = new Item(name, cost, multiplier: multiplier, description: description);
    ...
    public class Magic
    {
        public static Item CreateStaff(Item wood, Item essence)
        {
            ...
            Item staff = new Item(name, cost, multiplier: multiplier, description: description);
